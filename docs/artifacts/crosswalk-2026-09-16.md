---
layout: default
title: Crosswalk status — 2026-09-16
---

# Crosswalk status — 2026-09-16
...
| Road traffic deaths | 3.6.1 | 3 | RANK-ONLY | NO | 2021–2021 | grade RANK-ONLY: human judgment says these must not share an axis |
...
| Built-up area per capita (cities) | 11.7.1 | 1 | NO-SOURCE | NO | — | no NYC counterpart identified |
| Safely managed drinking water | 6.1.1 | 3 | NO-SIGNAL | NO | — | no NYC counterpart identified |
| Municipal waste collected | 11.6.1 | 3 | PROXY | NO | — | no overlapping years |
...
### Road traffic deaths (SDG 3.6.1) — **RANK-ONLY**, Tier 3

*NYC holds 11 years of rates from Vision Zero collision records; the UN holds ONE observation, 2021. That blocks a trend and not a ranking: WHO publishes a single global round, and 195 countries in that year is enough to place NYC (#18 of 196, 3.51 per 100k, beside Spain and the Netherlands, against 14.2 for the US). Originally graded BLOCKED -- the blocker was our framing, not the data. Still Tier 3: a city set against whole nations. WHO figures are modelled estimates; NYC counts collision records.*
...
- Overlap: 2021–2021 · units agree
  ...
  **Blockers:** grade RANK-ONLY: human judgment says these must not share an axis
...
- **NYC** `wvxf-dwi5` Housing Maintenance Code Violations — 17 obs, 2010–2026, updated 2026-09-15
...
### Built-up area per capita (cities) (SDG 11.7.1) — **NO-SOURCE**, Tier 1
...
### Municipal waste collected (SDG 11.6.1) — **PROXY**, Tier 3

*The US reports this indicator to nobody -- EN_MWT_COLLV has ZERO US observations in every year, while 90 other countries report it, so NYC's only comparator is the world. Both sides are derived to kg per capita, so the raw units differ by design (UN tonnes, NYC tons). TWO CAVEATS. DSNY collects from residences and institutions only and the UN definition includes commercial waste, so NYC's figure is an UNDERCOUNT and its true rank is worse -- the opposite direction to the recycling pair, where the same gap flatters the comparator instead. And DSNY publishes 'Tons' without saying short or metric; US short tons assumed, and if they are metric every NYC figure is ~10% low.*

- **UN** `undata/sdg/EN_MWT_COLLV` — 0 obs, unit ``, source —
- **NYC** `ebb7-mvp5` DSNY Monthly Tonnage Data — 22 obs, 2005–2026, updated 2026-09-10
- Overlap: — · unmapped UN unit `` - check by hand

  **Blockers:** no overlapping years

## Summary

**12 pairs · 3 chartable end to end.**
